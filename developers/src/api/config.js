...
export const userCbFactory = ($message, t) => (err) => {
  ...
  if (err.code === 401) {
    setTimeout(() => {
      ...
      window.location.href = `https://mixin.zone/oauth/authorize?client_id=${process.env.VUE_APP_CLIENT_ID}&scope=PROFILE:READ+APPS:READ+APPS:WRITE&response_type=code`;
    }, 100);
  }
};
